...
from udgp.geometry import *
from udgp.model import get_model
...
class Instance:
    ...
    def reset_with_core(
        ...
        while not core_found:
            ...
            if core_type == "mock":
                y_indices = self.rng.choice(self.n, n, replace=False)
                core_points = self.input_points[y_indices]

            elif core_type == "artificial":
                core_points = geom_molecule_artificial(n)
    ...
    def solve(
        ...
        *,
        ...
        for (i, j), k in model.assignments.items():
    ...
    def molecule_artificial(
        ...
        points = geom_molecule_artificial(n, rng)

        return cls.from_points(points, freq, rng=rng)
